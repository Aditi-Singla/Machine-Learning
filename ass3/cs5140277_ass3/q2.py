...
def train_model_sgd(data, w1, w2, b1, b2, alpha, num_iter):
	...
	for j in range(num_iter):
		...
		for i in range(m):
			...
			y_temp = y_train[i,0]
			(w1_grad, b1_grad, w2_grad, b2_grad) = get_grads_relu(x_temp, y_temp, w1, w2, b1, b2)
			w2 = w2 - (alpha*w2_grad)
			w1 = w1 - (alpha*w1_grad)
			b2 = b2 - (alpha*b2_grad)
			b1 = b1 - (alpha*b1_grad)
...
y = train_data[:,n-1]
for i in range(m):
	pred = compute_label_relu(np.matrix(np.transpose(X[i,:])), w1_out, w2_out, b1_out, b2_out)	
	if(y[i,0] == pred):
		acc = acc + 1
print(acc)
